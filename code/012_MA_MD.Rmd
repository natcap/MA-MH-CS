...
if (add_subgroup_analysis == T) {
  fname <- paste0('./data/0302-MA-output/', paste('ma_result.subgroup', mh_tool, '.rds', sep = '_')); fname
...
ma_result_all <- rbind(ma_result_panas, ma_result_poms,
                       ma_result.subgroup_panas, ma_result.subgroup_poms )
...
fname <- paste0(dir.fig, 'plot_es_combined', today, postfix_subgroup, '.png'); fname
...
len <- length(unique(p1$subgroup)); len
facet_decide <- ifelse(len >3, T, F)

p1 <- p1 %>%
  plot_effect_size_overall(data = ., subgroup = 'subgroup', facet_bygroup = facet_decide, show_legend = F)



p2 <- ma_result_all %>%
  dplyr::filter(subgroup != 'Overall', tool == 'POMS') 
len <- length(unique(p2$subgroup)); len
facet_decide <- ifelse(len >3, T, F)

p2 <- p2 %>%
  plot_effect_size_overall(data = ., subgroup = 'subgroup', facet_bygroup = facet_decide, show_legend = T) + 
...
ggarrange(p1 + rremove("x.title"), 
          ...
          heights = c(0.4, 1),
          align = "v")
fname <- paste0(dir.fig, 'plot_es_combined_subgroup', today, postfix_subgroup, '.png'); fname
