...
sub_group_var <- 'subgroup_design'
# sub_group_var <- 'exposure_o2'
# sub_group_var <- '' ## age group
# sub_group_var <- '' ## exposure duration 


if (add_subgroup_analysis == T) {
  byvar_subgroup <- sub_group_var
} else {
  byvar_subgroup <- NULL
}
...
for (sub_ind_i in sub_indicator_selected) {
  ...
  ma_smd <- meta::metacont(
    ...
    method.tau = "REML",
    hakn = TRUE, 
    subgroup = byvar_subgroup,  ## Grouping results by a variable, default = NULL
    title = mh_tool)
  ma_smd
