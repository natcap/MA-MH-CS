...
df_type_exp_tool %>% func_alluvial(
  ...
  labele_small = 5,
  show_y_ticks = F, 
  filename.prefix = '', filename.postfix = 'tool3_exposure_natureType')
```


## Prepare data for MA

### * Coefficient-focused outcomes ---
...
effect_size_ind_i <- c('coefficient')
...
## save for later use
f <- paste0('./data/0301-MA-input/', 'sub_', paste(mh_tool, sep = '', collapse = '_'), '.rds')
...
## save for later use
f <- paste0('./data/0301-MA-input/', 'sub_', paste(mh_tool, sep = '', collapse = '_'), '.RData')
save(mh_tool, exposure_type_i, effect_size_ind_i, exp_sub_df, file = f)
```
...
## save for later use
f <- paste0('./data/0301-MA-input/', 'sub_', paste(mh_tool, sep = '', collapse = '_'), '.rds'); f
saveRDS(exp_sub_df, file = f)
```
...
exp_sub_long_clean <- exp_sub_long %>%
  ...
  dplyr::mutate(
    ...
    ci95lower = ifelse(`Health outcome direction` == "Higher is better", -ci95lower, ci95lower),
    ) %>%
  ...
  dplyr::select(id, model_id, study_label, 
                all_of(cols_keep), 
...
# names(exp_sub_long_clean)
unique(exp_sub_long_clean$MH_tool_o1) %>% cat('\nUnique MH_tool_o1: ', ., '\n')
unique(exp_sub_long_clean$MH_tool_o2) %>% cat('\nUnique MH_tool_o2: ', ., '\n')

unique(exp_sub_long_clean$id) %>% length() %>% cat('\n', ., 'unique papers for', mh_tool, '\n')
```
...
```{r}
unique(exp_sub_long_clean$effect_size_indices)
  ...
  dplyr::mutate(
    mean = case_when(
      effect_size_indices %in% c("Standardized coefficients", "Unstandardized coefficients", "coefficient") & 
        `Health outcome direction` == "Lower is better" ~ -mean,
      effect_size_indices == 'or' & 
        `Health outcome direction` == "Lower is better" ~ 1-(mean-1),
      TRUE ~ mean),
    )
...
cat('\n\n')
unique(exp_sub_long_clean$MH_tool_o1) %>% cat('unique MH_tool_o1:', ., '\n')
unique(exp_sub_long_clean$MH_tool_o2) %>% cat('unique MH_tool_o2:', ., '\n')

if ( all(mh_tool %in% c('GHQ-12', "SF-12", "SF-36")) ) {
...
if ( any(mh_tool %in% c('GHQ-12', "SF-12", "SF-36")) ) {
  cat('\n\n')
  
  ##' inspect data for further cleaning 
  unique(exp_sub_mods_coef$MH_indicator_o1) %>% sort() %>% 
    paste(., collapse = '\t\n\t') %>%
    cat('MH_indicator_o1:\n\t', ., '\n');
  unique(exp_sub_mods_coef$MH_indicator_o2) %>% sort() %>% 
    paste(., collapse = '\t\n\t') %>%
    cat('MH_indicator_o2:\n\t', ., '\n\n');
  
  ## how many unique papers?
  unique(exp_sub_mods_coef$MH_tool_o1) %>% cat('unique MH_tool_o1:', ., '\n')
  unique(exp_sub_mods_coef$MH_tool_o2) %>% cat('unique MH_tool_o2:', ., '\n')
  
  length(unique(exp_sub_mods_coef$id)) %>% cat('\nFor', mh_tool, '\n In total, there are', ., 'unique papers.')
  nrow(exp_sub_mods_coef) %>% cat('\n In total, there are', ., 'studies. \n\n')
} else {
  cat('\n\n')
  
  ##' inspect data for further cleaning --------------------------------------------------
  unique(exp_sub_mods_md$MH_indicator_o1) %>% sort() %>% 
    paste(., collapse = '\n') %>%
    cat('MH_indicator_o1:\n\t', ., '\n\t');
  unique(exp_sub_mods_md$MH_indicator_o2) %>% sort() %>% 
    paste(., collapse = '\n') %>%
    cat('MH_indicator_o2:\n\t', ., '\n\t');
  
  ## how many unique papers?
  unique(exp_sub_mods_md$MH_tool_o1) %>% cat('unique MH_tool_o1:', ., '\n')
  unique(exp_sub_mods_md$MH_tool_o2) %>% cat('unique MH_tool_o2:', ., '\n')
