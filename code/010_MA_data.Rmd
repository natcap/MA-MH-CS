...
unique(exp_sub_long_clean$id) %>% length() %>% cat(., 'unique papers for', mh_tool, '\n')
```
...
exp_sub_mods <- exp_sub_long_clean %>%
  dplyr::mutate(
    mean = case_when(
      effect_size_indices %in% c("Standardized coefficients", "Unstandardized coefficients") & 
...
unique(exp_sub_long_clean$MH_tool_o1)
unique(exp_sub_long_clean$MH_tool_o2)
