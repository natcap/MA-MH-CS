...
df_q_ndvi <- df_q %>%
  filter(str_detect(nature_quantity, pattern = regex("NDVI|EVI|Percent", ignore_case = TRUE)))

df_q_ndvi_ <- df_q_ndvi %>%
  select(Tools, nature_quantity) %>%
...
fs <- list.files(path = './data/0301-MA-input/', pattern = '^sub_.*.RData', full.names = T)
fs
