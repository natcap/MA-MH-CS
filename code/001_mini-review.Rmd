...
dt <- df %>%
  ...
  dplyr::select(1:4, City, matches("Country|Mental health|Indicator|Tools")) %>%
  dplyr::select(!matches("duration", ignore.case = T)) %>%
  func_clean_indicatorsPro(data = ., column_name = 'Indicator') %>%
  func_clean_indicator_sub(data = ., column_name = 'Indicator', upper_case = F) %>%
...
# shp <- sf::st_transform(shp, robinson_crs)
...
func_ggsave(fname = fname, w = 6.5, h = 2.8)
```
