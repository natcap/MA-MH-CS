...
csv <- "review_278532_20231212051221.csv"     # 2023-12-11, 38 consensus done, add a new table to collect x, y info
...
dd.gsheet1 <- googlesheets4::read_sheet(link, sheet = 'Effect size Data Table-option1') %>%
  ...
  dplyr::mutate(mod_id = row_number() + 20 ) %>% ## extra data in addition to the Covidence Table
  dplyr::select(id, mod_id, everything()) %>%
  as.data.frame()

dd.gsheet2 <- googlesheets4::read_sheet(link, sheet = 'Effect size Data Table-option2') %>%
  group_by(id, Reviewer) %>%
  dplyr::mutate(mod_id = row_number() + 20 ) %>% ## extra data in addition to the Covidence Table
  dplyr::select(mod_id, everything()) %>%
  as.data.frame()
```
...
dd.gsheet1c <- dd.gsheet1 %>%
  ...
  # dplyr::select(1:Reviewer, mod_id, group_id, Control_Mean, Control_SD, -`Study ID`) %>%
  dplyr::select(id, group_id, mod_id, everything()) %>%
  arrange(id, Reviewer, mod_id) 
