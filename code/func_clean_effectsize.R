...
func_clean_effectsize <- function(
  ...
  d <- data %>%
    dplyr::mutate(
      ...
      ## text cleaning 
      !!column_name := gsub(" =", "", !!sym(column_name)), ## remove " = "
      ...
      !!column_name := gsub(" Cohen\\’s d\\(mean1 - mean2\\)\\/sd", "_cohens", !!sym(column_name)),
      !!column_name := gsub(" Cohen’s d\\/sd", "_cohens", !!sym(column_name)),

      !!column_name := gsub("Measure values before and after intervention", "Mean_pre_post", !!sym(column_name), ignore.case = T),
      !!column_name := gsub("Measure values between different interventions", "Raw values", !!sym(column_name), ignore.case = T),
      ...
      !!column_name := gsub("corr_linear|corr_logi|regression coefficient|Regression coefficient|coefficients", "coefficient", !!sym(column_name), ignore.case = T),
      !!column_name := gsub("Unstandardized coefficient|unstandardized_coef|unstandardized_coef", "coef_unstandardized", !!sym(column_name), ignore.case = T),
      !!column_name := gsub("unstandardized coef", "coef_unstandardized", !!sym(column_name), ignore.case = T),
