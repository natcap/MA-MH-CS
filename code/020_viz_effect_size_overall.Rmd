...
# subgroup_select <- 'age_group' ## age group
subgroup_select <- 'gender_group' ## age group
# subgroup_select <- 'duration_group' ## exposure 
...
if (subgroup_select == 'nature_type_o2') {
  colors_group1 <- func_color_nature_type(df = p1_tool, column_name = "subgroup")
  colored_group <- colors_nature_type
} else {
  colors_group1 <- brewer.pal(n = length(element_list1), "Set1")
  colored_group <- colors_group1
}
...
if (subgroup_select == 'nature_type_o2') {
  colors_group2 <- func_color_nature_type(df = p2_tool, column_name = "subgroup")
  colored_group <- colors_nature_type
} else {
  colors_group2 <- brewer.pal(n = length(element_list2), "Set1")
  colored_group <- colors_group2
}
...
p_comb <- 
  ggarrange(p1 + rremove("x.title"), 
            ...
            align = "v")
p_comb
fname <- paste0(dir.fig, 'es_combined_tool', length(mh_tool_list), postfix_group, '_', today, vv, '.png'); fname
...
fname <- paste0(dir.fig, 'es_combined_tool', length(mh_tool_list), postfix_group, '_', today, vv, '_.png'); fname
func_ggsave(fname, w = 6, h = 5.5+1, save_png = T)
```
