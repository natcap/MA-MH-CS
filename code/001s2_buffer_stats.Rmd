...
df_bf_ <- df_bf %>%
  ...
  dplyr::filter(buf_m > 0, buf_m < 10000) %>%
  as.data.frame()
...
p1 <- df_bf_ %>%
  ggplot(aes(x = buf_m)) +
  geom_histogram(binwidth = 100, color = "black", fill = "lightblue", alpha = 0.7, boundary = 0) +
  scale_x_continuous(breaks = seq(0, max(df_bf_$buf_m, na.rm = T), 500)) + 
  theme_classic() +
...
p2 <- df_bf_zomm %>%
  ...
  scale_x_continuous(
    breaks = seq(0, max(df_bf_zomm$buf_m, na.rm = T), by = 100),  # Bin edges as breaks
    labels = seq(0, max(df_bf_zomm$buf_m, na.rm = T), by = 100)  # Same for labels
  ) +
  theme_classic() +
