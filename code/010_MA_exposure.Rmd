...
## load data 
fs <- list.files(path = "./data/0301-MA-input/", pattern = '^df_covidenceFull', full.names = T); fs;
...
df_exp_l <- df_exp %>% 
  ...
  dplyr::mutate(
    ...
    exposure_type = ifelse(grepl(x = exposure_type, 'duration|Spending hour|Time spent', ignore.case = T), 'Duration in nature', exposure_type),
    ) %>%
...
###' subset papers that examined `exposure_type_i`
exp_sub <- df_exp_l %>%
  # dplyr::filter(grepl(exposure_type, pattern =  paste(exposure_type_i, collapse = "|"))) %>%
  dplyr::filter(exposure_type %in% exposure_type_i)
exp_sub_id <- unique(exp_sub$id)
