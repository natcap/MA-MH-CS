...
func_exp_subset <- function(df, exposure_type_i, mh_tool) {
  ...
  exp_sub_df <- df %>%
    ...
    ## subset cols -------------------------------------------------------------------------
    dplyr::rename("n_participants" = "Total number of participants") %>%
    dplyr::select(id:`Effect size indices`, effect_size_indices, 
                  everything()) %>%
    dplyr::select(id:Country, 
                  all_of(cols_keep), n_participants,
...
exp_sub_mod_l_clean <- exp_sub_mod_l %>%
  ...
  dplyr::select(id, all_of(cols_keep), 
                effect_size_indices:Mean_raw, Mean, 
                CI_95_lower_raw, CI_95_lower, everything()) %>%
