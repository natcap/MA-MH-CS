...
ma_cor_data <- exp_sub_mods_cor %>%
  dplyr::mutate(
    ##' further clean the nature types ---- ??? need to double-check original paper to
    ##'   correct these classes 
    nature_type_o1 = case_when(
      str_to_sentence(nature_type_o1) %in% c(
        "Other natural space/element", 
        "Public natural spaces") ~ "Greenspace",
      T ~ nature_type_o1
                )) %>%
...
func_color_nature_type(df = ma_result_all, column_name = 'subgroup')
...
ma_result_all %>%
  ...
  plot_effect_size_overall(data = ., 
                           color_var = 'subgroup', 
                           show_legend = T) +
  theme(legend.position = c(0, 0.1+v_just),
